...
@overload
def safe(*exceptions: type[Exception]) -> Callable[[Callable[..., Any]], Callable[..., Result[Any, Exception]]]: ...
...
def safe(*args: Any) -> Any:
    ...
    """
    if len(args) == 1 and callable(args[0]) and not (isinstance(args[0], type) and issubclass(args[0], Exception)):
